...
class TrainingProcessStage3():
    def __init__(self, config):
        self.config = config
        # load data
        self.train_rna_loaders, self.test_rna_loaders, self.train_atac_loaders, self.test_atac_loaders, self.training_iters = PrepareDataloader(
            config).getloader()
        ...
        self.criterion_encoding = EncodingLoss(
            dim=config.embedding_size, p=config.p, use_gpu=self.config.use_cuda)
        self.criterion_center = CenterLoss(
            num_classes=self.config.number_of_class, use_gpu=self.config.use_cuda, feat_dim=self.config.embedding_size)
    ...
    def adjust_learning_rate(self, optimizer, epoch):
        lr = self.config.lr_stage3 * \
            (0.1 ** ((epoch - 0) // self.config.lr_decay_epoch))
        ...
        if self.config.checkpoint is not None:
            if os.path.isfile(self.config.checkpoint):
                ...
                self.model_encoder.load_state_dict(
                    checkpoint['model_encoding_state_dict'])
                self.model_cell.load_state_dict(
                    checkpoint['model_cell_state_dict'])
            else:
                print("=> no resume checkpoint found at '{}'".format(
                    self.config.checkpoint))
    ...
    def train(self, epoch):
        ...
        for batch_idx in range(self.training_iters):
            ...
            for iter_rna_loader in iter_rna_loaders:
                ...
                # prepare data
                rna_data, rna_label = prepare_input(
                    [rna_data, rna_label], self.config)
                # model forward
            ...
            for iter_atac_loader in iter_atac_loaders:
                ...
                # prepare data
                atac_data, atac_label = prepare_input(
                    [atac_data, atac_label], self.config)
                # model forward
            ...
            if self.config.with_crossentorpy == True:
                cell_loss = self.criterion_cell(
                    rna_cell_predictions[0], rna_labels[0])
                for i in range(1, len(rna_cell_predictions)):
                    cell_loss += self.criterion_cell(
                        rna_cell_predictions[i], rna_labels[i])

                cell_loss = cell_loss/len(rna_cell_predictions)

                atac_cell_loss = self.criterion_cell(
                    atac_cell_predictions[0], atac_labels[0])
                for i in range(1, len(atac_cell_predictions)):
                    atac_cell_loss += self.criterion_cell(
                        atac_cell_predictions[i], atac_labels[i])

                cell_loss += atac_cell_loss/len(atac_cell_predictions)
            else:
                cell_loss = 0

            encoding_loss = self.criterion_encoding(
                atac_embeddings, rna_embeddings)
            center_loss = self.config.center_weight*(self.criterion_center(
                atac_embeddings, atac_labels) + self.criterion_center(rna_embeddings, rna_labels))
            ...
            progress_bar(batch_idx, self.training_iters,
                         'encoding_loss: %.3f, rna_loss: %.3f, center_loss: %.3f ' % (
                             total_encoding_loss /
                             (batch_idx + 1), total_cell_loss / (batch_idx +
                                                                 1), total_center_loss / (batch_idx + 1)
                         ))
        ...
        for i, rna_loader in enumerate(self.test_rna_loaders):
            ...
            for batch_idx, (rna_data, rna_label) in enumerate(rna_loader):
                ...
                rna_embedding = rna_embedding / \
                    norm(rna_embedding, axis=1, keepdims=True)
        ...
        for i, atac_loader in enumerate(self.test_atac_loaders):
            ...
            for batch_idx, (atac_data, atac_label) in enumerate(atac_loader):
                ...
                atac_embedding = atac_embedding / \
                    norm(atac_embedding, axis=1, keepdims=True)
                ...
                for print_i in range(test_num):
                    ...
                    for print_j in range(1, embedding_size):
                        fp_em.write(
                            ' ' + str(atac_embedding[print_i][print_j]))
                    fp_em.write('\n')
                ...
                for print_i in range(test_num):
                    ...
                    for print_j in range(1, prediction_size):
                        fp_pre.write(
                            ' ' + str(atac_cell_prediction[print_i][print_j]))
                    fp_pre.write('\n')
